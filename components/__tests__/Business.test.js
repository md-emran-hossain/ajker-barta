...
it('render Business component without crashing', () => {
  const div = document.createElement('div')
  ReactDOM.render(<Business />, div)
})

it('testing business with props', () => {
  const { getByTestId } = render(<Business testArr={testArr} />)
  expect(getByTestId('businessId')).toBeTruthy()
})
it('maching snapshot', () => {
  const tree = renderer.create(<Business />)
